...
<body>
	...
	<hr style="height:3px;" />
	<form action="${pageContext.request.contextPath }/insertTemplet.action?sort=${sortId }" method="post">
		<div>
			<div>
				<input type="text" name="dictTempletName" value=${dictTempletName } id="dictTempletName" placeholder="请输入字典模板名称">
			</div>
			<br />
			<br />
			<br />
			...
			<div>
				<div>
					<table border="1px" align="center" id="table_single">
						...
						<!-- 因“防止因为没有传值而报错“空指针错误””产生的问题，添加的片段在表格末尾，存储时出现错误，故使用标签固定添加的片段位置 -->
						<div id="table_single_-">
							
						</div>
						<c:forEach varStatus="i" var="dictSingle" items="${dictSingleList }">
							<tr id="single${i.count }">
								<td>
									${i.count }<textarea name="single_question" id="single_question_${i.count }" cols="40" rows="5">${dictSingle.dictSingleContent }</textarea>
								</td>
								<%
									n = 65;
								%>
									...
									<c:forEach var="dictSingleOption" items="${dictSingleOptionList }">
										<c:if test="${dictSingle.dictSingleId == dictSingleOption.dictSingleId }">
											<%
												temp = (char)n++;
											%>
											<%=temp %>.<textarea name="single_answer_<%=temp %>" id="single_answer_${i.count }_<%=temp %>" cols="70" rows="1">${dictSingleOption.dictSingleOptionContent }</textarea>
											<br />
										</c:if>
									</c:forEach>
									<%
								...
								<td>
									<a href=# onclick="delete_single(this)">删除</a>
								</td>
							</tr>
						</c:forEach>
					</table>
				...
				<div>
					<table border="1px" align="center" id="table_check">
						...
						<c:forEach varStatus="i" var="dictCheckbox" items="${dictCheckboxList }">
							<tr id="check${i.count }">
								<td>
									${i.count }<textarea name="check_question" id="check_question_${i.count }" cols="40" rows="5">${dictCheckbox.dictCheckboxContent }</textarea>
								</td>
								<%
									n = 65;
								%>
									...
									<c:forEach var="dictCheckboxOption" items="${dictCheckboxOptionList }">
										<c:if test="${dictCheckbox.dictCheckboxId == dictCheckboxOption.dictCheckboxId }">
											<%
												temp = (char)n++;
											%>
											<%=temp %>.<textarea name="check_answer_<%=temp %>" id="check_answer_${i.count }_<%=temp %>" cols="70" rows="1">${dictCheckboxOption.dictCheckboxOptionContent }</textarea>
											<br />
										</c:if>
									</c:forEach>
									<%
								...
								<td>
									<a href=# onclick="delete_check(this)">删除</a>
								</td>
							</tr>
						</c:forEach>
					</table>
				...
				<div>
					<table border="1px" align="center" id="table_text">
						...
						<c:forEach varStatus="i" var="dictText" items="${dictTextList }">
							<tr id="text${i.count }">
								<td>
									${i.count }<textarea name="text_question" id="text_question_${i.count }" cols="110" rows="7">${dictText.dictTextContent }</textarea>
								</td>
								<td>
									<a href=# onclick="delete_text(this)">删除</a>
								</td>
							</tr>
						</c:forEach>
					</table>
...
<script type="text/javascript">
	/* 防止因为没有传值而报错“空指针错误” */
	$(function(){
		$("#table_single_-").append("<tr hidden=hidden><td><textarea name=single_question>-</textarea></td><td><textarea name=single_answer_A>-</textarea><textarea name=single_answer_B>-</textarea><textarea name=single_answer_C>-</textarea><textarea name=single_answer_D>-</textarea></td></tr>");
		$("#table_check_-").append("<tr hidden=hidden><td><textarea name=check_question>-</textarea></td><td><textarea name=check_answer_A>-</textarea><textarea name=check_answer_B>-</textarea><textarea name=check_answer_C>-</textarea><textarea name=check_answer_D>-</textarea><textarea name=check_answer_E>-</textarea><textarea name=check_answer_F>-</textarea></td></tr>");
		$("#table_text_-").append("<tr hidden=hidden><td><textarea name=text_question>-</textarea></td></tr>");
	});
	/* 增加的单选题、多选题、文本题序号 */
	var i = ${dictSingleList.size() + 1 };
	var j = ${dictCheckboxList.size() + 1 };
	var k = ${dictTextList.size() + 1 };
	/* 记录被删除的题数，验证提交时使用 */
	var a = 0;
	var b = 0;
	var c = 0;
